...
CLOUDGATEWAY=$PREDIX_MACHINE_DATA/appdata/cloudgateway
...
echo $USE_JAVA  >> "$START_PREDIX_MACHINE_LOG" 2>&1
$USE_JAVA -version >> "$START_PREDIX_MACHINE_LOG" 2>&1
...
checkConnection () {
	...
	while [ $(date +%s) -lt $connection_end_time ] &&  [ ! -f "$CLOUDGATEWAY/connected" ]; do
	...
	if [ -f "$CLOUDGATEWAY/connected" ]; then
		...
		writeConsoleLog "Installation of $unzipDir succeeded. Return Code: $errorcode"
		installSuccess
	else
		...
		# Remove the status json from before because it will be over
		rm -vf "$PREDIX_MACHINE_DATA/appdata/packageframework/$unzipDir.json"
		rollback
		writeFailureJSON
		installFailed
	fi
...
while [ ! -f "$YETI_STOP_SIGNAL" ]; do
	...
	if [ "$zips" != "" ]; then
		for zip in "${PREDIX_MACHINE_DATA}"/installations/*.zip; do
			...
			$USE_JAVA -Xmx25m -jar "${PREDIX_MACHINE_DATA}"/yeti/com.ge.dspmicro.yetiappsignature-17.2.0.jar "${PREDIX_MACHINE_DATA}" "$zip">>"$START_PREDIX_MACHINE_LOG" 2>&1
			if [ $? -ne 0 ]; then
			...
			if [ $errorcode -eq 100 ]; then
				writeConsoleLog "Installation of $unzipDir succeeded.  An external process will write the status file when complete.  Return Code: $errorcode"
				installSuccess
				continue
			fi
			if [ $errorcode -ne 0 ]; then
				writeConsoleLog "Installation of $unzipDir failed. Error Code: $errorcode"
				installFailed
				continue
			fi
