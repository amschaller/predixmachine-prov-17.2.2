...
echo "predix.version=17.2.2" >> "$DIRNAME/predix.home.prs"
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/bcpkix-jdk15on-1.56.jar" ]; then
    EXTRA_CP="$EXTRA_CP:${PREDIX_MACHINE_HOME}/machine/bundles/bcpkix-jdk15on-1.56.jar"
fi

# Boot delegation requires this on an absolute path
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/bcprov-jdk15on-1.56.jar" ]; then
    EXTRA_CP="$EXTRA_CP:${PREDIX_MACHINE_HOME}/machine/bundles/bcprov-jdk15on-1.56.jar"
fi

# Add jSerialComm libraries for Modbus serial communication
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.machineadapter-opcua-17.2.1.jar" ]; then
    OPCUA_KEYSTORE_PATH=security/opcua_keystore.jks
fi
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.opcua-server-17.2.0.jar" ]; then
