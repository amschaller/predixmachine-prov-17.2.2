...
# Constants 
readonly MACHINE_BUILD_VERSION=17.2.2
...
PrintUsage () {
    ...
    echo "   ./DockerizeContainer.sh -m ../ --docker_host default --arch x86_64 --openvpn --container_name agent --tar_name PredixMachine --jre jre8 --http_proxy http://proxy-src.research.ge.com:8080 --https_proxy http://proxy-src.research.ge.com:8080 --no_proxy \"localhost,127.0.0.1,*.ge.com\""
    echo ""
    echo "REQUIRED:"
    ...
    echo "   --jre <JRE>                                The JRE to use in the image. Choices are only jre8 (default)"
    echo "   --openvpn                                  Option to build Predix Machine with OpenVPN installed."
    echo ""
...
ParseArguments () {
    ...
    while [[ $# -gt 0 ]]; do   
        case $1 in
            ...
            --openvpn)
                WITH_OPENVPN=true
                echo "OpenVPN Enabled Container: $WITH_OPENVPN"
                ;;
            --ftp_proxy)
                shift
                DOCKER_FTP_PROXY="$1"
...
DockerizeMachine () {
    ...
    if $WITH_OPENVPN; then
        OPENVPN="-openvpn"
    fi
    DOCKER_IMAGE_NAME=${DOCKER_IMAGE_PREFIX}-${ARCHITECTURE}:${MACHINE_BUILD_VERSION}-${CONTAINER_NAME}${OPENVPN}
    DOCKER_FILE_NAME=${DOCKER_FILE_PREFIX}-${ARCHITECTURE}-${JRE}${OPENVPN}

    if [ ! -z "$TAR_NAME" ]; then
        TAR_FILE=${TAR_NAME}-${CONTAINER_NAME}-${ARCHITECTURE}-${JRE}${OPENVPN}-${MACHINE_BUILD_VERSION}.tar
    else
        TAR_FILE=${MACHINE_FOLDER}-${ARCHITECTURE}-${JRE}${OPENVPN}.tar
    fi
