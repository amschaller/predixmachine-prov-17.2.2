...
# Copy new configs that are in ${PredixMachineDir}/configuration/machine iff this /data already exists with a config folder and the said files are not there. If the exist, do not overwrite.
# do this before following step - otherwise that step would affect integrity of this step
[ -d ${PREDIX_MACHINE_DATA_DIR}/configuration/machine -a -d ${PredixMachineDir}/configuration/machine ] &&
{
	for cfgfile in ${PredixMachineDir}/configuration/machine/*; do
	{
		if [ -f $cfgfile ] && [ ! -f ${PREDIX_MACHINE_DATA_DIR}/configuration/machine/$(basename $cfgfile) ]; then
		{
			cp $cfgfile ${PREDIX_MACHINE_DATA_DIR}/configuration/machine
		}
		fi
	}
	done
...
[ ! -d ${PREDIX_MACHINE_DATA_DIR}/appdata -a ! -d ${PREDIX_MACHINE_DATA_DIR}/configuration -a ! -d ${PREDIX_MACHINE_DATA_DIR}/logs -a ! -d ${PREDIX_MACHINE_DATA_DIR}/security -a ! -d ${PREDIX_MACHINE_DATA_DIR}/installations ] &&
{
	...
	[ ! -d ${PredixMachineDir}/openvpn -o ! -r ${PredixMachineDir}/openvpn ] &&
	{
...
# Check that the destination directory specified by PREDIX_MACHINE_DATA_DIR does not already contain the openvpn directory (which should not be overwritten)
[ ! -d ${PREDIX_MACHINE_DATA_DIR}/openvpn ] &&
{
	# Move the Predix Machine openvpn folder into the directory specified by PREDIX_MACHINE_DATA_DIR
	[ -d ${PredixMachineDir}/openvpn ] &&
	{
		mv ${PredixMachineDir}/openvpn	 		${PREDIX_MACHINE_DATA_DIR}
	}
}

# Check that the destination directory specified by PREDIX_MACHINE_DATA_DIR does not already contain the openvpn configuration directory (which should not be overwritten)
[ ! -d ${PREDIX_MACHINE_DATA_DIR}/configuration/openvpn ] &&
{
	# Move the Predix Machine openvpn configuration folder into the directory specified by PREDIX_MACHINE_DATA_DIR/configuration
	[ -d ${PredixMachineDir}/configuration/openvpn ] &&
	{
		mv ${PredixMachineDir}/configuration/openvpn	 		${PREDIX_MACHINE_DATA_DIR}/configuration
	}
}
...
rm -rf ${PredixMachineDir}/openvpn
